...
Recent commits use short, imperative subjects such as `Clarify in README: for example tic_tac_toe` and `Add scripts promote_run_to_stable.py`. Follow that style: one-line subject, present tense, focused scope. PRs should include a brief description, linked issue if applicable, exact commands used for verification, and screenshots only for GUI changes under `games/tic_tac_toe/`.
